...
**Минимум** (работает на процессоре):
- Windows 10/11 64-bit;
- 8 ГБ ОЗУ;
- ~10 ГБ на диске (среда ~5 ГБ + модели);
- любой современный многоядерный CPU — скорость примерно 1x от
  длительности записи (см. ориентиры ниже).

**Рекомендуется** (обработка в несколько раз быстрее реального времени):
- видеокарта NVIDIA с **4+ ГБ видеопамяти** — программа сама включит
  CUDA-ускорение, ничего настраивать не нужно;
- лучше всего **RTX 20xx и новее**: у них аппаратный float16
  (Compute Capability 7.0+), стандартный быстрый режим для GPU.
  На старых GTX 10xx/9xx CUDA тоже работает, но float16 у них
  медленный — движок откатится на float32, выигрыш будет скромнее;
- если диаризация тоже на GPU — комфортнее 6–8 ГБ видеопамяти
  (иначе её можно оставить на CPU отдельной настройкой);
- 16 ГБ ОЗУ и SSD — не обязательно, но приятно.

Карты AMD/Intel не поддерживаются CUDA-сборкой — на таких машинах
программа работает на процессоре.

## Установка и запуск

1. **Скачайте код** — зелёная кнопка «Code» → «Download ZIP» (или
   `git clone`) — и распакуйте в любую папку.
2. **Запустите `setup.bat`** — скрипт скачает и разложит среду:
   портативный Python + PyQt6, Faster-Whisper XXL, ffmpeg — и соберёт
   `Transcriber.exe`. Нужен интернет и ~5 ГБ места; занимает 5–15 минут
   (~1.6 ГБ загрузок). Повторный запуск безопасен: готовое пропускается,
   недокачанное докачивается. Версии зафиксированы в начале `setup.ps1`,
   загрузки удаляются автоматически после успешной сборки.
3. **Двойной клик по `Transcriber.exe`** — окно появится через 3–6 секунд.
   Модели Whisper докачаются сами при первой транскрибации.

Запасные варианты запуска: `run_gui.bat` (то же самое), `run_gui_debug.bat` —
...
**Как именно жмётся звук.** Кодеки — `libopus` (48 кГц) и `libmp3lame`
(44.1 кГц) из ffmpeg. Битрейты пресетов:

| Пресет | opus | mp3 |
|---|---|---|
| Компактное — голос | 32 kbps | 64 kbps |
| Стандарт — голос | 64 kbps | 128 kbps |
| Высокое — голос | 96 kbps | 192 kbps |
| Максимум — музыка | 256 kbps | 320 kbps |

Почему так: Opus — лучший из существующих кодеков для речи, разборчивость
сохраняется уже на 24–32 kbps, а на 64 kbps речь практически неотличима
от исходника — поэтому голосовые пресеты такие «маленькие», и именно
поэтому Opus используют Telegram/WhatsApp для голосовых сообщений.
MP3 при том же качестве требует примерно вдвое большего битрейта
(кодек 1993 года) — отсюда удвоенные цифры; он в списке только ради
совместимости со старыми плеерами и автомагнитолами.

Нормализация громкости — двухпроходный `loudnorm` (стандарт вещания
EBU R128): первый проход измеряет громкость файла, второй приводит её
к −16 LUFS — это общепринятый уровень для подкастов и речи. Потолок
пиков −1.5 dBTP защищает от клиппинга после сжатия. При сведении
многодорожечной записи каждая дорожка нормализуется отдельно **до**
сведения — тихий собеседник подтягивается к громкому, а не тонет в нём.
